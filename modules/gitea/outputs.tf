...
output "repo_url" {
  ...
  value       = "http://gitea-http.${var.namespace}.svc.cluster.local:${var.service_port}/${var.admin_user}/${var.repo_name}.git"
}

output "push_url" {
  description = "Host-reachable Git URL (with credentials) for `git push`."
  value       = "http://${var.admin_user}:${var.admin_password}@localhost:${var.host_port}/${var.admin_user}/${var.repo_name}.git"
  sensitive   = true
}
